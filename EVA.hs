...
ladjust threshold amount formula = case formula of
 Lcon c   -> formula
 Lvar i   -> (i .> threshold) (Lvar (i + amount)) formula
...
iadjust threshold amount formula = case formula of
 Icon c   -> formula
 Ivar i   -> (i .>= threshold) (Ivar (i + amount)) formula
...
lreduce = lstep (-1) where
 ...
 lstep :: Int -> Llambda -> (Llambda, Cbool)
...
ireduce (Iapp s t) = let
 ...
 in s_done (Iapp new_s t, true) (Iapp s new_t, t_done)
...
eval s = let (new_s, unfinished) = reduce s
         in unfinished (eval new_s) new_s

eval_show :: (Show a, Lambda a) => a -> IO ()
eval_show s = print s >>
 let (new_s, unfinished) = reduce s
 in unfinished (eval_show new_s) (return ())
